...
export default function AdminUsuariosPage() {
    ...
    const handleApproveUser = async (id: string) => {
        ...
        try {
            ...
            if (currentUser && !currentUser.allows_video && !currentUser.allows_text) {
                                        ...
                                        >
                                            ...
                                            <td className="px-6 py-5 text-right">
                                                <div className="flex items-center justify-end gap-1">
                                                    {['pending', 'waitlist', 'rejected'].includes(u.application_status || '') && (
                                                        <button 
                                                            onClick={() => handleApproveUser(u.id)}
                                                            disabled={isApproving === u.id}
                                                            className="p-2.5 text-green-400 hover:text-white hover:bg-green-500/20 rounded-xl transition-all disabled:opacity-50"
                                                            title="Aprovar Usuário"
                                                        >
                                                            {isApproving === u.id ? <div className="w-5 h-5 border-2 border-green-500 border-t-transparent rounded-full animate-spin" /> : <Check size={20} />}
                                                        </button>
                                                    )}
                                                    <button 
                                    ...
                                    <div className="flex gap-2 pt-2 border-t border-white/5">
                                        {['pending', 'waitlist', 'rejected'].includes(u.application_status || '') && (
                                            <button 
                                                onClick={() => handleApproveUser(u.id)}
                                                disabled={isApproving === u.id}
                                                className="flex-1 py-2.5 rounded-xl bg-green-500/10 text-green-400 text-xs font-bold hover:bg-green-500/20 transition-all flex items-center justify-center gap-2 disabled:opacity-50"
                                            >
                                                {isApproving === u.id ? <div className="w-4 h-4 border-2 border-green-500 border-t-transparent rounded-full animate-spin" /> : <Check size={16} />} 
                                                Aprovar
                                            </button>
                                        )}
                                        <button 
